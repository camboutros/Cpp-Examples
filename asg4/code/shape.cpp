...
text::text (void* glut_bitmap_name, const string& data) {
   DEBUGF ('c', this);
   textdata = data;
   glut_bitmap_font = glut_bitmap_name;
   //string name;
   //name = *(glut_bitmap_name);  
...
text::text(string* glut_bitmap_name, const string& data){
   textdata = data;
   string name = *glut_bitmap_name;
   auto i = fontcode.find(name);
   if (i == fontcode.end()){
      throw runtime_error("Not a font");
   } 
...
isoceles::isoceles(const GLfloat top, GLfloat bottom): 
      triangle({{0, top}, {bottom, 0}, {top, bottom}}){
}
